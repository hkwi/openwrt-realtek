...
)
{
	...
	if(!pDM_Odm->priv->pshare->rf_ft_var.edcca_thd)
		return;
	else if(pDM_Odm->RSSI_Min > pDM_Odm->priv->pshare->rf_ft_var.edcca_thd)
	...
	value32 = ODM_GetBBReg(pDM_Odm, 0xc50, 0xFFFF0000);
	pDM_Odm->TH_H = (u1Byte) (value32>>8);
	pDM_Odm->TH_L = (u1Byte) (value32 & 0xFF);
	...
	if(pDM_Odm->EDCCA_State == 1)
	{
		if(IGI < 0x28)
		{
			...
			if(TH_H_dmc >= 0xff)
				TH_H_dmc = TH_H_dmc -0xff; //Transform from negative to positive
			TH_L_dmc = pDM_Odm->TH_L + Diff;
			if(TH_L_dmc >= 0xff)	
				TH_L_dmc = TH_L_dmc - 0xff; //Transform from negative to positive
		}
		else
		{
			Diff = IGI - 0x28;
			if(Diff >= 3)
				Diff = 3; //May lose precision in low pwdb
			TH_H_dmc = pDM_Odm->TH_H - Diff;
			TH_L_dmc = pDM_Odm->TH_L - Diff;
		}
	}
...
#endif
	if(pDM_Odm->SupportICType & ODM_IC_11AC_SERIES)
		odm_DynamicEDCCA_AC(pDM_Odm, CurrentIGI);
			...
			else
			{
...
#endif
				{
					if((pDM_DigTable->CurIGValue >= 0x32) && (pFalseAlmCnt->Cnt_all > 1000))
					{
						...
						ODM_RT_TRACE(pDM_Odm,	ODM_COMP_DIG, ODM_DBG_LOUD, ("odm_DIG(): DIG one-shit to upperbound = 0x%x\n", CurrentIGI));
					}
					else
					{
		...
		for(i=0; i<ODM_ASSOCIATE_ENTRY_NUM; i++){
			PSTA_INFO_T pstat = pDM_Odm->pODM_StaInfo[i];
				...
				if(IS_STA_VALID(pstat)) {
					if(disable_pwr_ctrl)
						pstat->hp_level = 0;
					 else if ((pstat->hp_level == 0) && (pstat->rssi > TX_POWER_NEAR_FIELD_THRESH_8812))
							pstat->hp_level = 1;
						else if ((pstat->hp_level == 1) && (pstat->rssi < (TX_POWER_NEAR_FIELD_THRESH_8812-4)))
							pstat->hp_level = 0;
				}
			}
	}
	...
	int initial_gain)
{
	...
	ODM_SetRFReg(pDM_Odm, RF_PATH_A, RF_AC, bRFRegOffsetMask, 0x3bdb7); //?

	// Rx AGC off
	ODM_SetBBReg(pDM_Odm, 0x8c8, BIT0, 0); //8c8[0]
	ODM_SetBBReg(pDM_Odm, 0x8d8, BIT17, 0); //8d8[17]

	// Turn off CCA
	ODM_SetBBReg(pDM_Odm, 0x838, BIT3, 0x1); //838[3] 設為1

	// BB Reset
	ODM_SetBBReg(pDM_Odm, 0x808, BIT29, 0); //?
	ODM_SetBBReg(pDM_Odm, 0x808, BIT29, 1);  //?

	// initial_gain_psd &= 0x7f;
	pDM_Odm->bDMInitialGainEnable = FALSE;
	initial_gain_org = ODM_Read1Byte(pDM_Odm, 0xc50);
	ODM_Write_DIG(pDM_Odm, initial_gain); //setmib dig_enable 0;
	...
	// Rx AGC on
	ODM_SetBBReg(pDM_Odm, 0x8c8, BIT0, 1); //8c8[0]
	ODM_SetBBReg(pDM_Odm, 0x8d8, BIT17, 1); //8d8[17]
	// CCK on
	...
	ODM_Write_DIG(pDM_Odm, initial_gain_org); // 暫時不要加入
