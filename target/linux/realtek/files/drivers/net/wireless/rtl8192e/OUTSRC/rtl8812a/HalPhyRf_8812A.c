		...
		switch (Path) {
			...
			if (VDF_enable == 1){
				...
				for (k = 0;k <= 2; k++){
					...
					ODM_Write4Byte(pDM_Odm, 0xc88, 0x821603e1);
					ODM_Write4Byte(pDM_Odm, 0xc8c, 0x68163e96);
					ODM_Write4Byte(pDM_Odm, 0xcb8, 0x00100000);// cb8[20] 將 SI/PI 使用權切給 iqk_dpk module
					cal_retry = 0;
					while(1){
					...
					ODM_Write4Byte(pDM_Odm, 0xc80, 0x38008c15);// TX_Tone_idx[9:0], TxK_Mask[29] TX_Tone = 16
				   	ODM_Write4Byte(pDM_Odm, 0xc84, 0x18008c15);// RX_Tone_idx[9:0], RxK_Mask[29]
			...
			if (TX1IQKOK == FALSE)
				...
				for (k = 0;k <= 2; k++){
					...
					switch (k){
						...
						case 1:
							{
							ODM_Write4Byte(pDM_Odm, 0xe80, 0x08008c38);// TX_Tone_idx[9:0], TxK_Mask[29] TX_Tone = 16
							ODM_Write4Byte(pDM_Odm, 0xe84, 0x28008c38);// RX_Tone_idx[9:0], RxK_Mask[29]
							ODM_SetBBReg(pDM_Odm, 0xee8, BIT(30), 0x0);
							}
							break;
						case 2:
					...
					ODM_Write4Byte(pDM_Odm, 0xe88, 0x821603e1);
					ODM_Write4Byte(pDM_Odm, 0xe8c, 0x68163e96);
					ODM_Write4Byte(pDM_Odm, 0xeb8, 0x00100000);// cb8[20] 將 SI/PI 使用權切給 iqk_dpk module
					cal_retry = 0;
					while(1){
						...
						if (delay_count < 20){							// If 20ms No Result, then cal_retry++
			            	...
			            	if (TX1IQKOK == FALSE){   //If RX mode TXK fail, then take TXK Result
			                		TX_X1_RXK[cal] = TX_X0[cal];
			                		TX_Y1_RXK[cal] = TX_Y0[cal];
			                		TX1IQKOK = TRUE;
			                		ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("RXK Step 1 fail\n"));
			            	}
					...
					ODM_SetBBReg(pDM_Odm, 0x82c, BIT(31), 0x1); // [31] = 1 --> Page C1
					ODM_SetBBReg(pDM_Odm, 0xe80, BIT(29), 0x1); 
					ODM_SetBBReg(pDM_Odm, 0xe84, BIT(29), 0x0); 
		...
		)
{
	...
	u4Byte 	Backup_AFE_REG[AFE_REG_NUM] = {0xc5c, 0xc60, 0xc64, 0xc68, 0xc6c, 0xc70, 0xc74, 0xc78, 0xc7c, 0xc80, 0xc84, 0xcb8, 0xcb0, 0xcb4,
		       	                                                   0xe5c, 0xe60, 0xe64, 0xe68, 0xe6c, 0xe70, 0xe74, 0xe78, 0xe7c, 0xe80, 0xe84, 0xeb8, 0xeb0, 0xeb4}; 
