...
#define __ODMHWCONFIG_CALLEE_MAC
...
pu4Byte gArrayPointer_MAC_8192C;
...
{
	return gArrayPointer_MAC_8192C;
}
     ...
     )
{
	u2Byte len = odm_SetArrayPointerGetLength_8192C(pDM_Odm,
	                                             sizeof(Array_MAC_8192C)/sizeof(u4Byte),
	                                             Array_MAC_8192C,
	                                             &gArrayPointer_MAC_8192C
	                                            );
	return len;
}
 	...
 	)
{
	...
	u1Byte     interface   = pDM_Odm->SupportInterface;
	u1Byte     board       = pDM_Odm->BoardType;  
	u4Byte     ArrayLen    = sizeof(Array_MAC_8192C)/sizeof(u4Byte);
	pu4Byte    Array       = Array_MAC_8192C;
	...
	for (i = 0; i < ArrayLen; i += 2 )
	{
	    ...
	    if ( v1 < 0xCDCDCDCD )
	    {
	 		odm_ConfigMAC_8192C(pDM_Odm, v1, (u1Byte)v2);
		    continue;
	 	}
		else
		{ // this line is the start of branch
...
pu4Byte gArrayPointer_MAC_2T_8192C;
...
{
	return gArrayPointer_MAC_2T_8192C;
}
...
{
	u2Byte len = odm_SetArrayPointerGetLength_8192C(pDM_Odm,
	                                             sizeof(Array_MAC_2T_8192C)/sizeof(u4Byte),
	                                             Array_MAC_2T_8192C,
	                                             &gArrayPointer_MAC_2T_8192C
	                                            );
	return len;
}
 	...
 	)
{
	...
	u1Byte     interface   = pDM_Odm->SupportInterface;
	u1Byte     board       = pDM_Odm->BoardType;  
	u4Byte     ArrayLen    = sizeof(Array_MAC_2T_8192C)/sizeof(u4Byte);
	pu4Byte    Array       = Array_MAC_2T_8192C;
