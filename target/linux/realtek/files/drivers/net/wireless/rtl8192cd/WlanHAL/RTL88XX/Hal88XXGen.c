	...
	case HW_VAR_PHYREGFILE_HP_START:
	{
		...
		if ( IS_HARDWARE_TYPE_8192E(Adapter) ) {
			if (IS_HAL_TEST_CHIP(Adapter)) {
				...
				*((pu4Byte)(val)) = (u4Byte)data_PHY_REG_8192E_hp_start;
				//*((pu4Byte)(val)) = (u4Byte)data_PHY_REG_8192Emp_hp_start;
			}
		}
#endif            
	}
...
//#endif				
	...
	case HW_VAR_PHYREGFILE_AGC_HP_START:
	{
		...
		if ( IS_HARDWARE_TYPE_8192E(Adapter) ) {
			if (IS_HAL_TEST_CHIP(Adapter)) {
				...
				//printk("Get 8192E AGC_TAB_hp start\n"); 
				*((pu4Byte)(val)) = (u4Byte)data_AGC_TAB_8192E_hp_start;
			}
			else { // mp chip
				//printk("Get 8192E MP chip AGC_TAB_hp start\n"); 
				*((pu4Byte)(val)) = (u4Byte)data_AGC_TAB_8192E_hp_start;
				//*((pu4Byte)(val)) = (u4Byte)data_AGC_TAB_8192Emp_hp_start;
			}
		}
#endif            
	}
	...
	break; 
		...
		if ( IS_HARDWARE_TYPE_8881A(Adapter) ) {
			//*((pu4Byte)(val)) = (u4Byte)data_RadioA_8881A_hp_start;
            	...
            	if ( IS_HARDWARE_TYPE_8192E(Adapter) ) {
                	if (IS_HAL_TEST_CHIP(Adapter)) {
				//printk("Get 8192E RadioA_hp start\n");
    	                	*((pu4Byte)(val)) = (u4Byte)data_RadioA_8192E_hp_start;					
                	}
                	else {// mp chip
	                	//printk("Get 8192E MP chip RadioA_hp start\n");
	                    	*((pu4Byte)(val)) = (u4Byte)data_RadioA_8192E_hp_start;					                    
                	}
            	}
#endif            
	...
	case HW_VAR_RFREGFILE_RADIO_B_HP_START:
	{
		...
		if ( IS_HARDWARE_TYPE_8192E(Adapter) ) {
			if (IS_HAL_TEST_CHIP(Adapter)) {
				//printk("Get 8192E RadioB_hp start\n");
				*((pu4Byte)(val)) = (u4Byte)data_RadioB_8192E_hp_start; 				
			}
			else {// mp chip
				//printk("Get 8192E MP chip RadioB_hp start\n");
				*((pu4Byte)(val)) = (u4Byte)data_RadioB_8192E_hp_start; 	
				//*((pu4Byte)(val)) = (u4Byte)data_RadioB_8192Emp_hp_start; 									
			}
		}
#endif            
	}
...
)
{
	...
	if (HAL_IS_ROOT_INTERFACE(Adapter))
	{
        ...
        HAL_RTL_W8(REG_CCK_CHECK, (HAL_RTL_R8(REG_CCK_CHECK) | BIT(6)));
