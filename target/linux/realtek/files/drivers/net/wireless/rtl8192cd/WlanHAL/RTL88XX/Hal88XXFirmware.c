...
)
{
	...
	switch(BoxNum)
	{
		case 0:
			...
			BOXRegLast = REG_HMEBOX_3;
			BOXExtRegLast = REG_HMEBOX_E3_V1;
			break;
		case 1:
			BOXReg = REG_HMEBOX_1;
			BOXExtReg = REG_HMEBOX_E1_V1;
			BOXRegLast = REG_HMEBOX_0;
			BOXExtRegLast = REG_HMEBOX_E0_V1;
			break;
		case 2:
			BOXReg = REG_HMEBOX_2;
			BOXExtReg = REG_HMEBOX_E2_V1;
			BOXRegLast = REG_HMEBOX_1;
			BOXExtRegLast = REG_HMEBOX_E1_V1;
			break;
		case 3:
			BOXReg = REG_HMEBOX_3;
			BOXExtReg = REG_HMEBOX_E3_V1;
			BOXRegLast = REG_HMEBOX_2;
			BOXExtRegLast = REG_HMEBOX_E2_V1;
			break;
		default:
			break;
	}
	...
	while(!bFwReadClear)
	{
		...
		if(WaitH2cLimmit == 600) { //do the first stage, clear last cmd
			...
			HAL_RTL_W32(REG_HMEBOX_0, 0x00020101);
			HAL_RTL_W32(REG_HMEBOX_1, 0x00020101);
			HAL_RTL_W32(REG_HMEBOX_2, 0x00020101);
			HAL_RTL_W32(REG_HMEBOX_3, 0x00020101);
...
#if	1
		if(pEntry->tx_bw > Adapter->pshare->CurrentChannelBW)
			pEntry->tx_bw = Adapter->pshare->CurrentChannelBW;
#endif	
...
//			ratr_bitmap &= 0x3FCFFFFF;
		...
		if (Adapter->pshare->is_40m_bw && (pstat->tx_bw == HT_CHANNEL_WIDTH_20_40)
				...
				&& (Adapter->bg_ap_timeout || Adapter->force_20_sta || Adapter->switch_20_sta
#ifdef STA_EXT
				|| Adapter->force_20_sta_ext || Adapter->switch_20_sta_ext
#endif
				))
#endif
		)
