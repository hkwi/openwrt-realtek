	...
	)
{
	if (Method == BBSWING)
	{
		...
		pDM_Odm->BbSwingIdxOfdm[RF_PATH_A] = (pDM_Odm->BbSwingIdxOfdm[RF_PATH_A] > 29) ? 29 : pDM_Odm->BbSwingIdxOfdm[RF_PATH_A];
		pDM_Odm->BbSwingIdxOfdm[RF_PATH_B] = (pDM_Odm->BbSwingIdxOfdm[RF_PATH_B] > 29) ? 29 : pDM_Odm->BbSwingIdxOfdm[RF_PATH_B];		
		
		// Adjust BB swing by Tx scaling, no matter CCK or OFDM.
		if (RFPath == RF_PATH_A)
			ODM_SetBBReg(pDM_Odm, rA_TxScale_Jaguar, 0xFFE00000, TxScalingTable_Jaguar[pDM_Odm->BbSwingIdxOfdm[RF_PATH_A]]);
		else if (RFPath == RF_PATH_B)
			ODM_SetBBReg(pDM_Odm, rB_TxScale_Jaguar, 0xFFE00000, TxScalingTable_Jaguar[pDM_Odm->BbSwingIdxOfdm[RF_PATH_B]]);
	}
	else
	{
		return;
	...
	)
{
	...
	u1Byte         	band  	  		 = pHalData->CurrentBandType;
	...
	case ODM_RF_PATH_A:
		{
			...
			ODM_SetBBReg(pDM_Odm, 0xc10, 0x000003ff, RX_X);
			ODM_SetBBReg(pDM_Odm, 0xc10, 0x03ff0000, RX_Y&0x000003ff);
			ODM_RT_TRACE(pDM_Odm,ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("RX_X = %x;;RX_Y = %x ====>fill to IQC\n", RX_X, RX_Y&0x000003ff));
	...
	)
{
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("BandWidth = %d, ExtPA = %d\n", *pDM_Odm->pBandWidth, pDM_Odm->ExtPA));
	...
	case ODM_RF_PATH_A:
       {
		...
		for (i = 0; i < RX_Average; i++){
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("RX_X0[%d] = %x ;; RX_Y0[%d] = %x\n", i, (RX_X0[i])>>22&0x000003ff, i, (RX_Y0[i])>>22&0x000003ff));
		}
		for (i = 0; i < RX_Average; i++){
			for (ii = i+1; ii <RX_Average; ii++){
				dx = (RX_X0[i]>>22) - (RX_X0[ii]>>22);
				DbgPrint("dx = %d\n", dx);
				if (dx < 3 && dx > -3){
					dy = (RX_Y0[i]>>22) - (RX_Y0[ii]>>22);
					DbgPrint("dy = %d\n", dy);
						if (dy < 3 && dy > -3){
							RX_X = ((RX_X0[i]>>22) + (RX_X0[ii]>>22))>>1;
							RX_Y = ((RX_Y0[i]>>22) + (RX_Y0[ii]>>22))>>1;
							RX_finish = 1;
							break;
						}
				}
		...
		if (RX_finish == 1){
			...
			_IQK_RX_FillIQC_8821A(pDM_Odm, Path, 0x100, 0x0);
		}
		}
		break;
	default:
	...
	)
{
	...
	u4Byte 	Backup_MACBB_REG[MACBB_REG_NUM] = {0x520, 0x550, 0x808, 0x838, 0x90c, 0xb00, 0xc00, 0xc50, 0xe00, 0xe50 , 0x82c}; 
